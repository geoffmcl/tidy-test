/*
 ==============================================================================================================
 tidy-buf-test.c - HTML TidyLib command line driver

 MODIFIED: stdout replaced with a buffer allocation
 See BUG: https://sourceforge.net/p/tidy/bugs/404/
 Test case from: 01/04/2016  19:46 638 tidySaveString.zip
 DEBUG: -config F:\Projects\tidy-test\test\input5\cfg_sf404.txt F:\Projects\tidy-test\test\input5\in_sf404.html
 ==============================================================================================================
 */
...
static tmbstr get_option_names( const CmdOptDesc* pos )
{
    ...
    len = (uint)strlen(localPos.name1);
    if (localPos.name2)
        len += (uint)(2+strlen(localPos.name2));
    if (localPos.name3)
        len += (uint)(2+strlen(localPos.name3));
 ...
 */
#ifdef TIDY_TEST    // building tidy-test
static void version( void )
{
#ifdef PLATFORM_NAME
    printf( "HTML Tidy for %s, circa %s, version %s\n",
             PLATFORM_NAME, tidyReleaseDate(), tidyLibraryVersion() );
#else
    printf( "HTML Tidy circa %s, version %s\n", tidyReleaseDate(), tidyLibraryVersion() );
#endif
}
...
int main( int argc, char** argv )
{
    ...
    while ( argc > 0 )
    {
        ...
        {
            printf("tidy-buf-test: No input html file found!\n");
            return -1;
            //htmlfil = "stdin";
        ...
        if ( status >= 0 && tidyOptGetBool(tdoc, TidyShowMarkup) )
        {
            ...
            else
            {
                ...
                if ( outfil ) {
                    ...
                    do 
                    {
                        status = tidySaveString( tdoc, buffer, &buflen );
                        ...
                        if (status == -ENOMEM)
                        {
                            ...
                            if (buffer)
                                MemFree(buffer);
                            buffer = (tmbstr)MemAlloc(buflen + 1);
                            if (!buffer) {
